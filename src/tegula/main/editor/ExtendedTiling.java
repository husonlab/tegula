...
import tegula.main.collection.SimpleTiling;
import tegula.tiling.EuclideanTilingMeshes;
import tegula.tiling.HyperbolicTilingMeshes;
import tegula.tiling.TilingMeshesBase;
...
public class ExtendedTiling extends SimpleTiling {
    ...
    public void decreaseTiling() {
        if (tilingMeshes instanceof HyperbolicTilingMeshes) {
            final HyperbolicTilingMeshes hyperbolicTilingMeshes = (HyperbolicTilingMeshes) tilingMeshes;
    ...
    private void translateTilingHyperbolic(double dx, double dy) {
        if (tilingMeshes instanceof HyperbolicTilingMeshes) {
            ...
            if (false) {
                ...
                if (newTiles == TilingMeshesBase.FAILED) { // Generates new tiling if too many rounding errors
    ...
    private void translateTilingEuclidean(double dx, double dy) {
        if (tilingMeshes instanceof EuclideanTilingMeshes) {
            final EuclideanTilingMeshes euclideanTilingMeshes = (EuclideanTilingMeshes) tilingMeshes;
            ...
            if (newTiles == TilingMeshesBase.FAILED) { // Generates new tiling if too much rounding errors
                resetTiling(); // Reset fundamental domain
                update(); // Update tiling
            } else { // No rounding errors: add new tiles
                all.getChildren().addAll(newTiles);
    ...
    public void replaceTiling(DSymbol dSymbol) {
        tilingMeshes = TilingMeshesBase.createTiling(tilingStyle, dSymbol);
        update();
    }
