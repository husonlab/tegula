...
public class ReshapeManager {
     ...
     * @return handles
     */
    public Collection<Node> createHandles(double factor) {
        ...
        for (int k = 0; k <= 2; k++) {
            ...
            for (int a = 1; a <= size; a = ds.nextOrbit(i, j, a, visited)) {
                if ((ds.getVij(i, j, a) == 1) && !(i == 0 && j == 2 && a == ds.getS2(a))) {
                    final Shape shape = makeReshapeHandle(factor, fDomain.getVertex3D(k, a), ds.isCycle(i, j, a) ? Color.GREEN : Color.YELLOW);
                    handles.add(shape);
                    setMouseHandler(factor, shape, k, a);
                } else {
                    handles.add(makeReshapeHandle(factor, fDomain.getVertex3D(k, a), Color.RED));
                }
            }
        }
        ...
        for (int a = 1; a <= size; a++) {
            if (a < ds.getS2(a)) {
                final Shape shape = makeReshapeHandle(factor, fDomain.getEdgeCenter3D(2, a), Color.GREEN);
                handles.add(shape);
                setMouseHandler(factor, shape, 2, a);
            } else if (a == ds.getS2(a)) {
                handles.add(makeReshapeHandle(factor, fDomain.getEdgeCenter3D(2, a), Color.DARKRED));
            }
        }
        return handles;
    }
     ...
     */
    private static Shape makeReshapeHandle(double factor, Point3D v, Color color) {
        final Circle shape = new Circle(4);
        shape.setLayoutX(factor * v.getX());
        shape.setLayoutY(factor * v.getY());
     ...
     * @param deltaY
     */
    public Point2D resetShape(double deltaX, double deltaY, final int k, final int a) {
        // Reset Point in fundamental domain
        Point2D transVector = new Point2D(deltaX, deltaY);

        if (k <= 1) {
            final int length = ds.computeOrbitLength(1 - k, 2, a);
            // Add restrictions:
            if (k == 1) {
            ...
            transVector = addMirrorRestriction(transVector.getX(), transVector.getY(), length, k, a); // Mirror axis restriction

            final Translate translate = new Translate(transVector.getX(), transVector.getY());

            // Translate Point of type i in chamber a

            Point3D pt = fDomain.getVertex3D(k, a);
            ...
            // Consider all points in orbit of a (especially if chamber contains boundary edges)
            {
                ...
                for (int z = 1; z <= length; z++) {
                    ...
                    if (fDomain.isBoundaryEdge(1 - k, b)) {
                        final Transform g = generators.get(1 - k, b);
                        {
                            pt = g.transform(pt);
                            pt2d = Tools.map3Dto2D(fDomain.getGeometry(), pt);
                            fDomain.setVertex(pt2d, k, ds.getSi(1 - k, b));
                        }
                    }
                    b = ds.getSi(1 - k, b);

                    // If 2-edge is on boundary
                    if (fDomain.isBoundaryEdge(2, b)) {
                        final Transform g = generators.get(2, b);
                        pt = g.transform(pt);
                        pt2d = Tools.map3Dto2D(fDomain.getGeometry(), pt);
                        fDomain.setVertex(pt2d, k, ds.getSi(2, b));
                    }
                    b = ds.getSi(2, b);
                }
            }
     ...
     * @return
     */
    private Point2D addMirrorRestriction(double deltaX, double deltaY, int orbitLength, int k, int flag) {
        ...
        ds.markOrbit(1 - k, 2, flag, visited);
        ...
        if (2 * orbitLength / numberOfChambers == 2) { // Condition for exactly one mirror axis
            while (flag <= m) {
                if (fDomain.isBoundaryEdge(1 - k, flag) && ds.getSi(1 - k, flag) == flag) { // Mirror axis is always a boundary for fundamental domain
                    Transform g = generators.get(1 - k, flag);
                    n = fDomain.getVertex3D(1 - k, flag).subtract(g.transform(fDomain.getVertex3D(1 - k, flag))); // Orthogonal direction of mirror axis
                    r = new Point3D(n.getY(), -n.getX(), 0); // Direction of translation for handle = direction of mirror axis
                    Point3D transformed = g.transform(fDomain.getVertex3D(1 - k, flag));
                    q = fDomain.getVertex3D(1 - k, flag).midpoint(transformed);

                    break;
                } else {
                    flag = ds.getSi(1 - k, flag);
                }
            ...
            Point3D oldPos = fDomain.getVertex3D(k, flag);
     ...
     */
    private void setMouseHandler(double factor, Shape shape, int k, int a) {
        ...
        shape.setOnMouseDragged((e) -> {
            ...
            final Point2D contraintsAdjustmentTranslationVector = resetShape(deltaX / factor, deltaY / factor, k, a);
